...
async function main(): Promise<void> {
  ...
  const setup = async (): Promise<void> => {
    ...
    async function deployUsdc(chain: Network): Promise<void> {
  ...
  const bridge = async () => {
    ...
    const testerWallet: Wallet = new ethers.Wallet(pk); // does wallet need to be connected with provider?
    ...
    const ethRpcUrl = "http://localhost:8500";

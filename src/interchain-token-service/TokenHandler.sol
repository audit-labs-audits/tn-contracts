...
/// @notice Unchanged from Axelar ITS TokenHandler v2.1.0 except giveToken, takeToken are virtual for TEL-specific override
...
contract TokenHandler is ITokenHandler, ITokenManagerType, ReentrancyGuard, Create3AddressFixed {
     ...
     */
    function giveToken(bytes32 tokenId, address to, uint256 amount) external virtual returns (uint256, address) {
    ...
    function takeToken(bytes32 tokenId, bool tokenOnly, address from, uint256 amount) external virtual payable returns (uint256) {
        address tokenManager = _create3Address(tokenId);
        (uint256 tokenManagerType, address tokenAddress) = ITokenManagerProxy(tokenManager).getImplementationTypeAndTokenAddress();

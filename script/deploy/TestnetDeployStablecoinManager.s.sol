...
contract TestnetDeployStablecoinManager is Script {
    ...
    function run() public {
        ...
        (bool implRes, ) = deployments.ArachnidDeterministicDeployFactory.call(
        ...
        (bool proxyRes, bytes memory proxyRet) = deployments
            .ArachnidDeterministicDeployFactory
            .call(DETERMINISTIC_FAUCET_PROXY_DATA);
        require(proxyRes);
        address payable stablecoinManagerAddress = payable(
            address(bytes20(proxyRet))
        );
        ...
        bytes memory setNativeDripAmountCall = abi.encodeWithSelector(
            StablecoinManager.setNativeDripAmount.selector,
            nativeDripAmount
        );
        stablecoinManager.upgradeToAndCall(
            address(stablecoinManagerImpl),
            setNativeDripAmountCall
        );
        ...
        stablecoinManager.UpdateXYZ(
            stablecoinManager.NATIVE_TOKEN_POINTER(),
            true,
            maxLimit,
            minLimit
        );
        ...
        for (uint256 i; i < stables.length; ++i) {
            Stablecoin(stables[i]).grantRole(
                minterRole,
        ...
        for (uint256 i; i < faucets.length; ++i) {
            assert(
                stablecoinManager.hasRole(
                    stablecoinManager.FAUCET_ROLE(),
                    faucets[i]
                )
            );
        }
        ...
        vm.writeJson(
            LibString.toHexString(
                uint256(uint160(address(stablecoinManager))),
                20
            ),
            dest,
            ".StablecoinManager"

...
contract TestnetDeployTNBridgeContracts is Script {
    ...
    function setUp() public {
        ...
        domainSeparator = keccak256(
            abi.encodePacked(axelarIdForTelcoin, routerAddress, telChainId)
        );
    ...
    function run() public {
        ...
        // minimumRotationDelay)`
        axelarAmplifierImpl = AxelarAmplifierGateway(
            CREATE3.deployDeterministic(gatewayImplInitcode, gatewayImplSalt)
        );

        // construct contract init code for gateway proxy
        WeightedSigners memory weightedSigners = WeightedSigners(
            signerArray,
            threshold,
            nonce
        );
        WeightedSigners[] memory weightedSignersArray = new WeightedSigners[](
            1
        );
        ...
        axelarAmplifier = AxelarAmplifierGateway(
            CREATE3.deployDeterministic(gatewayProxyInitcode, gatewayProxySalt)
        );
        ...
        assert(
            axelarAmplifier.implementation() == address(axelarAmplifierImpl)
        );
        ...
        vm.writeJson(
            LibString.toHexString(
                uint256(uint160(address(axelarAmplifierImpl))),
                20
            ),
            dest,
        ...
        vm.writeJson(
            LibString.toHexString(
                uint256(uint160(address(axelarAmplifier))),
                20
            ),
            dest,
